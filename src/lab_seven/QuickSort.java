...
public class QuickSort {
    ...
    public static <T extends Comparable<T>> void sortRandom(T[] table) {
        quicksortRandom(table, 0, table.length - 1);
    }
    ...
    private static <T extends Comparable<T>> void quicksortRandom(T[] table, int first, int last) {
        if (first < last) {
            int pivotIndex = partitionRandom(table, first, last);
            quicksort(table, first, pivotIndex - 1);
            quicksort(table, pivotIndex + 1, last);
    ...
    private static <T extends Comparable<T>> int partitionRandom(T[] table, int first, int last) {
        //做一步随机化
        int random = first + (int) ((last - first + 1) * Math.random());
        swap(table, last, random);
        //取table[last]为快排基准
